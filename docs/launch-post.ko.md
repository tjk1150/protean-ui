...
- 알려진 공백, 숨기지 않습니다: 판단 축은 현재 크기 x 입력 2개뿐이고, 기본 정책은 입력
  ...
  태블릿 분화는 데이터 확보 후). 컨테이너 단위 트레이트와 가상 키보드 트레이트는
  로드맵입니다. 시트 모드 Select는 업스트림에 비앵커 옵션이 생길 때까지 CSS로 포지셔너를
...
- 문서와 라이브 데모: [LIVE_DEMO_URL]
- 왜 만들었나: [LIVE_DEMO_URL]/ko/why · 설계 원리: [LIVE_DEMO_URL]/ko/concepts/design-principles
