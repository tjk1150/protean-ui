...
- 오버레이 사용처 기준 앱 코드 73% 감소 (55줄 -> 15줄; 래퍼 라이브러리가 아니라 손으로
  쓴 레시피 대비입니다).
- 번들: 열 역할 전부 합쳐 react 6.9KB gzip, core 1.2KB (Base UI 제외). 역할 하나만
  쓰면 서브패스(`@protean-ui/react/dialog`)로 3.0KB이고, 이 트리셰이킹은 릴리스
  게이트가 계약으로 지킵니다.
- 테스트 225개. 기본 정책은 63셀 결정 테이블로 전수 검증되고, 경계값·상태 전이·예외
