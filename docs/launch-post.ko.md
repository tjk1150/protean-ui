...
구조적으로 틀릴 수 없게 설계했고, 오버레이 - 셀렉트 - 메뉴 - 내비게이션 - 화면 뼈대 -
주 행동 - 툴팁 - 리스트·디테일 - 액션 줄 - 보조 패널, 열 역할을 같은 정책으로 다룹니다. (참고로 Credenza는 2025년 11월 이후 멈춰 있고 npm에
배포된 적이 없습니다.)
...
- 판정의 기준은 뷰포트만이 아닙니다. `ProteanBoundary`로 감싼 영역 안에서는 그 패널의
  ...
  축입니다.
- 설치 한 줄로 앱 전체가 됩니다. Base UI가 peer로 함께 설치되므로, 적응이 필요 없는
  나머지 컴포넌트(폼 컨트롤, 탭, 토스트 등)는 그대로 import해서 씁니다 - 다른 UI
  라이브러리를 더 깔 필요가 없고, Protean을 걷어내면 평범한 Base UI 앱이 남습니다.
...
- 오버레이 사용처 기준 앱 코드 73% 감소 (55줄 -> 15줄; 래퍼 라이브러리가 아니라 손으로
  쓴 레시피 대비입니다).
- 번들: 열 역할 전부 합쳐 react 7.1KB gzip, core 1.3KB (Base UI 제외). 역할 하나만
  쓰면 서브패스(`@protean-ui/react/dialog`)로 3.1KB이고, 이 트리셰이킹은 릴리스
  게이트가 계약으로 지킵니다.
- 테스트 245개. 기본 정책은 72셀 결정 테이블로 전수 검증되고, 경계값·상태 전이·예외
...
- 프리알파입니다. 오늘 존재하는 역할은 열: Dialog, Select, Menu, Navigation, Screen,
  PrimaryAction, Tooltip, ListDetail, Actions, SupportingPane. React 18+, Next.js App Router와 Vite 모두
  1급 타깃.
