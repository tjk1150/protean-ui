...
- 오버레이는 열리는 순간 결정합니다(원하면 `continuity="live"`로 열림 중 재판정 - 이때도 콘텐츠 DOM·입력 상태·포커스가 보존된 채 모습만 바뀝니다). 서버가 보낸 HTML에는 트리거만 있고 오버레이 마크업이
...
- 오버레이 사용처 기준 앱 코드 73% 감소 (55줄 -> 15줄; 래퍼 라이브러리가 아니라 손으로
  쓴 레시피 대비입니다).
- 번들: react 4.4KB gzip, core 1.1KB gzip (Base UI 제외). 결정 레이어의 전체 무게입니다.
...
- 스타일은 강요하지 않되 방치하지도 않습니다: 값이 뷰포트가 아니라 presentation에 붙는
  참고 스타일시트(`@protean-ui/react/reference.css`)를 동봉합니다. 시트는 위쪽 모서리만
  둥글고 풀스크린은 모서리가 없는 이유가 media query가 아니라 역할 전환입니다. 전부
  `@layer`라 여러분의 CSS가 항상 이깁니다.
- 알려진 공백, 숨기지 않습니다: 판단 축은 현재 크기 x 입력 2개뿐이고, 기본 정책은 입력
  축을 compact에서만 따집니다(실기기 검증 없는 추측 규칙을 기본값에 넣지 않기 위해 -
  태블릿 분화는 데이터 확보 후). 가상 키보드는 트레이트로 수집되고(기본 팩은 아직 사용하지 않음), 오버레이 결정은
  ProteanBoundary로 컨테이너 단위 스코프가 가능합니다(1440px 모니터의 420px 패널 안
  다이얼로그는 compact로 판정되고, 시트는 패널 자체의 바닥에서 올라오며 스크림도
  패널만 덮습니다 - 열리는 순간 측정이라 SSR 불변식 그대로). 컨테이너 스코프
  크롬은 CSS 컨테이너 쿼리의 몫으로 남습니다. 시트 모드 Select는 업스트림에 비앵커 옵션이 생길 때까지 CSS로 포지셔너를
  고정합니다(항목이 많으면 searchable로 정식 콤보박스 패턴의 검색 입력이 팝업 안에
  들어갑니다). 태블릿 레일 셀과 바 오버플로 셀은
  jsdom과 iOS 시뮬레이터(실제 WebKit - iPhone 16 Pro, iPad Pro)에서 검증했고, 물리
  기기 검증은 남아 있습니다.
