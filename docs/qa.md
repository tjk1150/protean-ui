...
|---|---|---|
| Functional suitability | Decisions match the documented policy, exactly | `packs.decision-table.test.ts` - the full 72-cell cartesian table |
...
| Maintainability | The decision layer is pure and testable without a DOM | `@protean-ui/core` has no React/DOM dependency; 123 node tests |
...
|---|---|---|
| Decision table | The app-first pack, exhaustively: 3 overlay roles x 3 sizes x 3 inputs, plus navigation, primary action, hint, list-detail, and density (72 cells) | `core/src/packs.decision-table.test.ts` |
