export default function AccessibilityKo() {
  return (
    <div className="doc" lang="ko">
      ...
      <h2>측정한 결과</h2>
      <ul>
        <li>내비게이션의 모든 모습과 전체 화면 다이얼로그에서 axe 위반 0건이었어요.</li>
        <li>
          모달이 열린 상태에서는, 같은 부품으로 만든 수동 레시피보다 지적 항목이 오히려
          1개 적었어요. 남은 항목은 양쪽 모두에 있는 백엔드의 포커스 가드로, 알려진 검사
          특성이에요.
        </li>
        <li>
      ...
      <p>
        입력 수단이 바꾸는 건 버튼 크기나 호버 스타일 같은 겉모습뿐이에요. 키보드 조작은
        어떤 모습에서도 항상 돼요. 태블릿에 키보드를 연결한 사용자도 있으니까요. 동작
        줄이기(prefers-reduced-motion) 설정도 판단 기준에 포함돼 있어서 애니메이션이
        한곳에서 함께 줄어들어요.
      </p>
    </div>
  )
